...
- ✅ **Audit log + rate limit, *ephemeral*** — every chat turn appends `{ts, user, prompt_head, tools}` to a JSONL audit file at `/tmp/lsms_audit.log`; in-process sliding-window bucket per typed identifier (default 30 turns/hour, 200/day) caps damage from a leaked password. **Both reset on every Space restart** until `AUDIT_LOG_PATH` is pointed at persistent storage; *"user"* here is whatever email someone types into the Chainlit login, not a verified identity (see *missing*, below).
